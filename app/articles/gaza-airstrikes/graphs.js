...
import { FullscreenControl, GeolocateControl, Map, Marker, NavigationControl } from "@vis.gl/react-maplibre";
        ...
        <div className="sticky top-0 w-1/2">
            <div className="items-center w-[90%] h-[90lvh]">
                <Text paragraph>
                    ...
                    # of cumulative airstrikes: {gazaData.reduce((acc, curr) => acc + (dayjs(curr.event_date).isBefore(dayjs(date)) ? 1 : 0), 0)}
                    # of cumulative fatalities: {gazaData.reduce((acc, curr) => acc + parseInt(dayjs(curr.event_date).isBefore(dayjs(date)) ? curr.fatalities : 0), 0)}
                </Text>
